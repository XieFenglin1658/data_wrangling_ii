...
import NSDUH data from the web \##把网站上的所有信息都导入到R
...
\####CSS(Cascading Style
Sheets,层叠样式表)是网页设计中用于控制布局和外观的强大工具。tag（选择器）是CSS的核心,它允许我们精确地定位HTML元素并应用样式
...
Get the NYC water consumption dataset \##通过HTTP package
...
<img src="reading_data_from_the_web_files/figure-gfm/unnamed-chunk-10-1.png" width="90%" />

Access BRFSS

\##如果需要 app token，则去该网站找到token，按照一下操作即可

\##因为数据集实在太大了，我不想每次knit都重新跑一遍他(完成一遍full API
call)，所以`eval=FALSE`
...
Look at Pokemon data \##这些数据太complicated，无法快速使用“parsed”
